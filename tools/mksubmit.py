...
# Derived from classes.py rather than hand-listed: a class appears here as soon
# as it has a builder on disk, with no second place to remember to update.
PACKS = {
    c.slug: (c.name, c.builder,
             [("All Specs", f"{c.slug}-all-specs.txt")] +
             [(c.spec_label(s), f"{c.slug}-{s}.txt") for s in c.specs])
